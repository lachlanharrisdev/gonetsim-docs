...
State maps strings to binary-safe strings — no tables, no serialization. It's deliberately bounded: keys are capped at 4 KiB, values at 1 MiB, and everything (all scopes, all listeners combined) draws from one total budget, `64MiB` by default, configurable with `state.total_limit`. When a `set()` would push total state past the budget it fails gracefully — `set` returns `false` plus a reason instead of throwing, so scripts can react or carry on.

This is a resource guarantee rather than a sandbox restriction: state is bounded so a handler cannot consume unbounded simulator memory.

```lua
-- one greeting per client, remembered across connections
